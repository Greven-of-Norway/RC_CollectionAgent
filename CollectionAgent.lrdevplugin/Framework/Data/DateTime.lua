...
function DateTime:parseIptcDateTime( iptcDateTime )
    ...
    if #halfs == 2 then
        local dateComps = str:split( halfs[1], "-" ) -- interpreted as plain text by default.
        if #dateComps == 3 then
            ...
            if #timeComps == 3 then
                ...
                if #secStr == 3 then -- ###2 could be >3?
                    secStr = secStr:sub( 1, 2 ) -- time zone? ###2
                end
                second = tonumber( secStr )
                if year and month and day and hour and minute and second then
                    return LrDate.timeFromComponents( year, month, day, hour, minute, second, "local" ) -- ###2 time-zone?.
                else
