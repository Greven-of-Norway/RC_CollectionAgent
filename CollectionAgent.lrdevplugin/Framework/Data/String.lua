...
--
function String:getRoot( path ) -- ###2 test on Mac.
...
function String:getDriveAndSubPath( path )
    ...
    if #leaves > 1 then -- there are 2 parts
        ...
        return root, table.concat( leaves, WIN_ENV and "\\" or "/" ) -- standardize file-path... ###2 test on Mac
    else
        assert( path == root, "?" )
...
function String:child( s1, sep, s2 )
    ...
    if str:isBeginningWith( s2, sep ) then -- until 25/Apr/2014 2:47 was sep was interpreted as regex, now plain text (###2 hope nothing breaks).
