--[[
        ...
        This module aims at being able to serialize/deserialize (lua->xml, xml->lua) java objects created/decreated using 
        Simple XML Serialization class available here: http://simple.sourceforge.net/download/stream/doc/examples/examples.php
        
        The original purpose is to be able to have a version of Intercom that can more easily communicate with java apps.
        In other words, messages are serialized/de-serialized on plugin side using this lua code, and on java side using
        the above mentioned parser/deparser.
--]]

local XmlSimple, dbg, dbgf = Object:newClass{ className = 'XmlSimple', register = true }



--- Constructor for extending class.
--
function XmlSimple:newClass( t )
...
--                              
--
function XmlSimple:_parseAttrs(s)
  local args = {}
...
--                              - empty = 1, or text.
--
function XmlSimple:_tabularize(s)
    local stack = {}
...
--
function XmlSimple:_serializeAttributes( attrs )
    local args = ''
...
function XmlSimple:_spaces( level )
...
function XmlSimple:_escape( original )
...
--  Presently only used for lua serializer, although could be adapted for general use.
--
function XmlSimple:_unescape( escaped )
...
function XmlSimple:xmlEscape( s )
...
--		                    - recursive.
--
function XmlSimple:_serializeLua( elemName, luaValue, indentLevel, attrStr )
    local s
    indentLevel = indentLevel or 0
    local sp = self:_spaces( indentLevel )
    if type( luaValue ) == 'table' then
        if #luaValue == 0 then -- struct
            s = str:fmtx( '^2<^3^1>\n', attrStr or "", sp, elemName )
            for k, v in pairs( luaValue ) do
                s = s .. self:_serializeLua( k, v, indentLevel + 1, "" ) -- str:fmt( ' i="^1" t="^2"', str:to( k ), type( k ) ) )
            end
            s = s .. str:fmt( '^1</^2>\n', sp, elemName )
        else -- array
            s = str:fmtx( '^2<^3^1>\n', attrStr or "", sp, elemName )
            local itemName = elemName:sub( 1, #elemName - 1 ) -- ###
            for i, v in pairs( luaValue ) do
                s = s .. self:_serializeLua( itemName, v, indentLevel + 1, "" ) -- str:fmt( ' i="^1" t="^2"', str:to( k ), type( k ) ) )
            end
            s = s .. str:fmt( '^1</^2>\n', sp, elemName )
        end
    else -- assume no functions are being serialized.
        s =  str:fmt( '^3<^4^2>^1</^4>\n', self:xmlEscape( luaValue ), "", sp, elemName )
    end
    dbgf( s )
    return s
end
...
--
function XmlSimple:newDocument( xml, includePreamble )
    if xml ~= nil then
...
--
function XmlSimple:serializeLua( rootElemName, luaT, includeDecl, nSpaces )
    self.nSpaces = nSpaces or 2
    local s = self:_serializeLua( rootElemName, luaT )
    if includeDecl then
...
--
function XmlSimple:parseToLua( xmlString, expectDecl )

    --app:error( "###2" ) -- I'm not sure the status of this function @26/Feb/2014 11:01.

    local _xt = self:_tabularize( xmlString )
    Debug.pause( xmlString, _xt )
    if _xt == nil then
        return nil
    end
    if expectDecl then
    ...
    local function convertElement( elem, luaT )
        ...
        if tab:isArray( elem:getChildren() ) then
            local tbl = {}
            ...
            value = elem:getText() -- may need to figure out type somehow ###2
        end
...
--
function XmlSimple:parseXml( x, decl )
...
--
function XmlSimple:serialize( xdoc, initialIndentationSpaceString, numberOfSpacesPerIndentLevel )
