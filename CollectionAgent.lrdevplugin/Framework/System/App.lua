...
function App:changeFileDates( params )
    ...
    if WIN_ENV then
        ...
        elseif params.createdTime then
            ...
            if creTime == params.createdTime then -- true equality too exacting? ###2 could use num--is-within.
            ...
            if preservePreviousModifiedTime then
                ...
                if modTime2 ~= modTime then -- ###2 this is good if only desire is to change created time, but if mod time doesn't change then ftp sync app may not know to re-upload - hmm. ###2
        ...
        if self.bezelScope then
            ...
            app:pcall{ name="Showing old-style bezel", async=true, main=function( call ) -- guard=App.guardSilent - no need to guard,since bezel-scope acts as "gate".
                self.bezelScope = LrDialogs.showModalProgressDialog {
                    ...
                    -- @another now: not working in Lr5.2 - go figure. ###2 CC 800-833-6687 conf: 561 178 44
                }
...
--- Clear bezel.
--
function App:clearBezel() -- ###2 not sure how this works on Lr4/3
...
function App:alertLogWE( opts, fmt, ... )
    ...
    -- ###2 Different subject... - I think I need an auto-clear option - to clear log file upon each service invocation - would that work?
...
--
--  @usage Use in contexts when it makes sense for message to stay until user acknowledges it by clicking 'X' in progress scope, namely background process (which actually have their own display method which may be preferred(?), since it's clearable) or non-modal dialog boxes,
