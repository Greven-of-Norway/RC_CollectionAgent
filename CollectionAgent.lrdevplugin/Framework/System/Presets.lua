...
--  ###2 this could prove tricky, since actual pref is a table member, not a simple entity - hmmmm... - but maybe the key will just be a reference to the table element.


-- ###2 for emphasis: the success of this new scheme depends on how it works out to observe changes to structured prefs.
...
function Presets:_getPrefKey( propName, presetName )
    ...
    return p.prefs[propName] -- this won't do - it'll return the value instead. ###2 uh-oh...
...
function Presets:loadProps( props )
    ...
    for k,v in prefs:pairs() do
        if str:isStartingWith( k, prefix ) then
            ...
            -- ###2 change handler considerations?
