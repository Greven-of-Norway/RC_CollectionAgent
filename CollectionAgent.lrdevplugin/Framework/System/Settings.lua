                ...
                elseif value == "__del__" then
                    ...
                    if names ~= nil and #names > 0 then
                        ...
                        if index then
                            ...
                            -- adjust values table: (note: this clause added 11/Jun/2014, all plugins using array settings need to be re-released ###2.
                    ...
                    if str:is( name ) then
                        if not nameSet[name] then
                            ...
                            elseif not exists then
                                app:logW( "Export preset does not exist: ^1", path ) -- errors thrown are not being optimally handled in all contexts (results in "no settings", but no explanation) ###2, so make sure a warning is logged instead.
                            else -- directory
...
local function getExportPresetItems()
    ...
    local function doFile( dirName, file )
        ...
        --Debug.pauseIf( dirName ~= dirName2, "?" ) -- dir-name-2 *is* equal to dir-name, or has been so far..
        local fileName = LrPathUtils.leafName( file )
...
function Settings:getViewItems( params )
    ...
    local function closeTab()
        ...
        viewLookup[specId] = tabs[#tabs] -- ?###2 (not sure the lookup is doing any good...).
...
--]]
        ...
        elseif spec.dataType == 'number' then
            ...
            elseif spec.viewType == 'smartCollChooser' then
                ...
                cat:getSmartCollectionPopupItems( nil ) -- dump items - just init (###). Start where? (nil => catalog) ###2
                ...
                local button = vf:push_button {
                    ...
                    action = function( button )
                        app:call( Call:new{ name="Smart Collection Chooser", async=true, main=function( call )
                            ...
                            local items, lookup = cat:getSmartCollectionPopupItems() -- ditto ###2
