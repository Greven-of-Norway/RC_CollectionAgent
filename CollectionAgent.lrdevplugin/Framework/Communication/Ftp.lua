--[[
        ...
        ###2 warning: *some* (incomplete) strides have been made so ftp settings can be mapped - long story short: if settings are not using standard mapping, take caution (and consider just using standard names if possible).
--]]
...
function Ftp:new( t )
    ...
    if o.ftpSettings and o.ftpSettings.path then -- ###2 no mapping
...
function Ftp:init( ftpSettings, autoNegotiate )
    ...
    if self.ftpSettings and self.ftpSettings.path then
        self.ftpSettings.path = Ftp.formatPath( self.ftpSettings.path ) -- ###2 unmapped.
    end
...
function Ftp:connect()
    ...
    if self.ftpSettings and self.ftpSettings.path then -- unmapped ###2
...
--  ###2 @12/Mar/2014 21:13 ftp-settings expects server unmapped.
                            ...
                            if options.dateTol then -- assure date within tolerance to be considered "same".
                                if ( attrs.fileModificationDate - fileEntry.date ) > options.dateTol then
                                    if app:isAdvDbgEna() then
                                        local server = ( self.ftpSettings or {} )['server'] or 0 -- ###2 unmapped
