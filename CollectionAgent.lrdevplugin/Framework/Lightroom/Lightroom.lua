...
function Lightroom:prepareForRestart( catPath )
    ...
    local s, m = app:call( Call:new{ name="Preparing to Restart Lightroom", async=false, main=function( call ) -- no guarding "should" be necessary.
        ...
        if WIN_ENV then
            ...
            f = function()
                return app:executeCommand( "open", nil, targets ) -- ###2 test on Mac - @10/May/2013 17:20 - not validated on Mac.
            end -- no qual
            --]]
        end
    end } )
...
function Lightroom:exit()
    ...
    if WIN_ENV then
        ...
        s, m = app:sendMacEncKeys( keys ) -- ###2 test on Mac.
    end
    return s, m
end
...
function Lightroom:computeLrAppPath()
    ...
    for i, v in ipairs( presetFolders ) do
        if v:getPath():find( "Adobe Photoshop Lightroom" ) then
            ...
            local lrAppPath = LrPathUtils.child( lrAppFolderPath, WIN_ENV and 'lightroom.exe' or 'lightroom' ) -- ###2 test on Mac.
