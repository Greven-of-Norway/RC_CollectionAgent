...
DevelopSettings.table = { -- static table with all info for all settings.
    ...
    {   groupName = "Tone Curve",
        members={
            ...
            -- rel-constraint=0 added 29/Nov/2014 4:13, which changes handling - this is documented on Gazoo page.
...
function DevelopSettings:assurePv2012BasicSettings( photo, dev, tmo, atFlag, expSettings )
    ...
    repeat
        ...
        if count == ( maxCount / 2 ) then -- half way..
            -- ### NOTE: this clause added 11/Jan/2015 3:22 unconditionally because it's required for reliable settling, *BUT* it means calling context must save photo selection and restore afterward if desirable.
