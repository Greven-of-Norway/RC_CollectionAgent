    ...
    repeat
        if not saveStarted then
            if not oldWay and app:lrVersion() >= 5 then
                -- Note: there was a bug in Mac version until 3/Sep/2013 10:24 when not saving old-way in Lr5, since math-floor was not what was being recorded for time (see clause below). ###2 all plugins doing metadata save on Mac should be re-released.
                if WIN_ENV then
...
function Catalog:getPhotosInSource( anySource, assumeSubfoldersToo, ignoreIfNotTop, ignoreIfBuried )
    ...
    if self.photosInSource and self.photosInSource[anySource] then
        ...
        if app:isAdvDbgEna() then
            ...
            -- assert( #photosInSourceCached == 0 or type( photosInSourceCached[1] ) == 'table', "?" ) -- this assertion commented out for efficiency 19/Jan/2015 16:41.
    ...
    local function loadPhotosFromSource( source )
        if source.getPhotos then
            if self.photosInSource and self.photosInSource[source] then -- pre-validated photos already in cache for specified source.
                for i, p in ipairs( self.photosInSource[source] ) do -- ###2 perhaps not optimal coding..
    ...
    if false and app:isAdvDbgEna() then -- seems to be working fine - computed and cached are equivalent.
        if photosInSourceCached then
            local eq, x = tab:isEquivalent( photosInSourceCached, photosInSourceComputed )
            if not eq then
                error( x )
            -- else remove this non-sense after proven.###
            end
        -- else first time computation
        end
    end
    -- assert( #photosInSourceComputed == 0 or type( photosInSourceComputed[1] ) == 'table', "?" ) - presumably this assertion is no longer needed or beneficial, so.. (faster is better..).
...
function Catalog:getVisiblePhotos( params )
    -- works in Lr3+4+5/win7; ###2 test on Mac.
    params = params or {}
...
function Catalog:setCollectionPhotos( coll, photos, tmo )
    ...
    if #photosToAdd > 0 or #photosToRmv > 0 then
        ...
        if catalog.hasWriteAccess then
            s, m = LrTasks.pcall( setCollPhotos )
        else
            local name = self:getSourceName( coll )
            s, m = self:update( tmo or 20, str:fmtx( "Setting '^1' collection photos (^2)", name, #photos ), setCollPhotos )
        end
        if not s then
            error( m )
        end
...
--  @usage Dependency: pref named saveXmpBeforeRemovingFromCatalog (non-global) can be initialized prior to calling. 
--
...
function Catalog:removePhotos( params )
    ...
    if not tab:isArray( rmvPhotos ) then
        ...
        return true -- I guess.. - another option would be to suppress the warning and switch to a [ s, m ] return pair.
    end
    ...
    if s then
        ...
        local saveXmp = app:getPref( 'saveXmpBeforeRemovingFromCatalog' )
        if saveXmp then
    ...
    for i, photo in ipairs( rmvPhotos ) do
        local uuid = photo:getRawMetadata( 'uuid' )
        local tp = catalog:findPhotoByUuid( uuid ) -- cheap insurance..
        if tp == photo then -- photo passed the integrity/sanity check (still in catalog with specified uuid).
            uuids[#uuids + 1] = uuid
        else
            app:logV( "Can't remove specified photo - it's not in the catalog, uuid: ^1", uuid )
        end
    end
    if #uuids ~= #rmvPhotos then
...
function Catalog:deletePhotos( params )
    ...
    if s then
        ...
        local prompt
        local apk
        local tb2 = ", and you should be viewing thumbnail grid in Library module"
        if WIN_ENV then
        ...
        elseif final then
            ...
            okButton = "notOk"
            logsButton = 'ok'
            apk = promptTidbit .. " deletion confirmation" -- ###2 seems a bit wonked - consider usage..
        else
            mDelButton = 'ok'
        ...
        if WIN_ENV then
            ...
            -- local s, m = app:sendMacEncKeys( "CmdOptionShift-Delete" ) -- ###2 test on Mac (I believe the modifiers are correct, the 'Delete' part is a guess).
        end
        return true
    else
        app:logError( m )
...
function Catalog:getSmartCollection( id )--, startHere )
    if not self.smartCollLookup then
        ...
        getFromSet( catalog ) -- maybe presumptuous(?) ###2.
    end
...
--
--  @usage Initializes for both techniques: sqliteroom compatible file, and preset-based.
--  @usage Which techinique is used depends on which were successfully initialized and preference of calling context.
--
...
--- Get smart collections associated with specified photo.
--
