...
--      <br>    cache (Cache, optional) if passed, raw contents should include 'path', 'fileFormat', and not sure what else (use debug mode to ascertain).
...
function EmbeddedPreviews:getEmbeddedPreview( params )
    ...
    local jpgExt = LrPathUtils.extension( jpgPath )
    local rawExt = LrPathUtils.extension( rawPath )
    assert( not str:isEqualIgnoringCase( jpgExt, rawExt ), "jpeg extension must not be same as raw extension" ) -- cheap insurance (calling context could conceivably use the same extension, then rename, but let's call that a bad idea..).
    local jpgParent = LrPathUtils.parent( jpgPath )
    local s, m, c = fso:assureDir( jpgParent )
    if s then -- assured
        if c then -- created.
            app:logV( "Created directory for embedded preview extraction: ^1", jpgParent )
        -- else - already existed - stay quiet.
        end
    else -- failed
        Debug.pauseIf( not m, "?" )
        return false, str:fmtx( "Unable to create directory for embedded preview extraction - ^1", m or "no error message provided" )
    end
    ...
    local function extract()
        ...
        if fso:existsAsFile( jpgPath ) then -- a file at specified target path already exists.
            local deleted, wasntIt = fso:deleteFile( jpgPath ) -- cant re-get embedded preview at that path until existing file is deleted.
            if not deleted then
                return false, str:fmtx( "Unable to get embedded preview due to error: ^1", wasntIt )
            end
        end
    ...
    local function transferMetadata()
        ...
        local s, m = image:transferMetadata {
            ...
            ets = ets, -- ###2 @19/Jan/2015 18:33 it seems this must be exiftool emulating a session - not sure why a session object isn't supported, but I think there was a problem.
        }
        if s then
