...
--- Copy a folder (tree) or a file. ###2 not yet tested.
--      
--  <p>One motivation: to handle case of silent failure of Lr version.</p>
--                      
...
function Disk:readFile( filePath )
    ...
    --[[ the original way - cons: does not support non-ascii chars in path, *and* uses excessive memory (and is slow) for very large (e.g. video) files.
        ...
        return contents, msg
    --]]
    -- the new way (note: I've flipped back and forth a couple times in the hopes of better reliability - dunno whether checking readability is a key, or read method - ugh. ###2
...
-- f unction D isk:m oveFile( sourcePath, destPath, createDestDirsIfNecessary, overwriteDestFileIfNecessary, avoidUnnecessaryUpdate, timeCheckIsEnough )
        ...
        else -- ###2 this error is (was?) happening intermittently, e.g. when auto-publishing to Photooey & TreeSync simultaneously. Perhaps they are both trying to write the same temp file?
    ...
    local function isExcl( f )
        for i, v in ipairs( excl ) do
            ...
            if fc == '*' then
                ...
                if lc == '*' then
                    mt = v:sub( 1, v:len() - 1 )
                    fd = str:isStartingWith( f, mt ) -- *** note: this is NOT plain text (but if I fix here it might break elsewhere - hmm..). ###2
                    if fd then
                        return true
                    end
                else
