...
--- Determine if minimal metadata being exported.
--
--  @param settings (table, optional) if not passed, in-object export-params must be initialized.
--
function Export:isMinMeta( settings )
    if settings == nil then
        if self.exportParams then
            return ( app:lrVersion() <= 3 and self.exportParams.LR_minimizeEmbeddedMetadata ) or ( self.exportParams.LR_embeddedMetadataOption == 'copyrightOnly' )
        else
            error( "no settings", 2 )
        end
    else
        return ( app:lrVersion() <= 3 and settings.LR_minimizeEmbeddedMetadata ) or ( settings.LR_embeddedMetadataOption == 'copyrightOnly' )
    end
end
...
function Export:getSettingsFromPreset( params )
    ...
    for k, value in pairs(_G.s.value) do
        local is = str:isStartingWith( k, exportServiceProvider ) --       ###2 25/Apr/2014 2:38 - do I really want regex here?
...
-- private/protected method to assure filename preset cache is populated.
--
function Export:_assurePresetCache( props, photo )
    if self.filenamePresetCache == nil then
        ...
        -- post 13/Apr/2014 2:57 ###2 not fully tested / released.
    ...
    if not props.LR_renamingTokensOn then -- universal:
        ...
        if not self.filenamePreset then
            ...
            if not self.filenamePreset then
                ...
                --Debug.lognpp( "lookup", self.filenamePresetLookup ) -- ###2
